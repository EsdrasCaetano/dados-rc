...
export function Cenografia() {
  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['cenografia'] !== undefined && row['cenografia'] !== '' && parseInt(row['cenografia']) !== 0);
        ...
        const ageCounts: AgeCount = filteredData.reduce((acc: AgeCount, row: Row) => {
         ...
         // Ordena por idade de forma crescente
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <h2>Cenografia (Palco, artes, performers)</h2>
