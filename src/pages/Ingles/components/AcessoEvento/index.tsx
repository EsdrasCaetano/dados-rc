  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['en_acesso_evento'] !== undefined && row['en_acesso_evento'] !== '' && parseInt(row['en_acesso_evento']) !== 0);
        ...
        const ageCounts: AgeCounts = filteredData.reduce((acc: AgeCounts, row: Row) => {
          const age = row['en_acesso_evento'];
          acc[age] = (acc[age] || 0) + 1;
          return acc;
        }, {});
