...
export function DestinoCarneiros() {
  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['en_destino_carneiros'] !== undefined && row['en_destino_carneiros'] !== '' && parseInt(row['en_destino_carneiros']) !== 0);
        ...
        const ageCounts: AgeCounts = filteredData.reduce((acc: AgeCounts, row: Row) => {
          const age = row['en_destino_carneiros'];
          acc[age] = (acc[age] || 0) + 1;
          return acc;
        }, {});
