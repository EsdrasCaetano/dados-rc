...
export function EstruturaDoEvento() {
  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['en_estrutura_evento'] !== undefined && row['en_estrutura_evento'] !== '' && parseInt(row['en_estrutura_evento']) !== 0);
        ...
        const ageCounts: AgeCounts = filteredData.reduce((acc: AgeCounts, row: Row) => {
          const age = row['en_estrutura_evento'];
          acc[age] = (acc[age] || 0) + 1;
          return acc;
        }, {});
