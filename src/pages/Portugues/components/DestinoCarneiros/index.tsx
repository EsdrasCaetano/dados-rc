  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['destino_carneiros'] !== undefined && row['destino_carneiros'] !== '' && parseInt(row['destino_carneiros']) !== 0);
        ...
        const ageCounts: AgeCount = filteredData.reduce((acc: AgeCount, row: Row) => {
          const age = row['destino_carneiros'];
          acc[age] = (acc[age] || 0) + 1;
          return acc;
        }, {});
