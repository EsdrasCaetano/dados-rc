...
export function BeachClubVoar() {
  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['en_beach_club_voar'] !== undefined && row['en_beach_club_voar'] !== '' && parseInt(row['en_beach_club_voar']) !== 0);
        ...
        const ageCounts: AgeCounts = filteredData.reduce((acc: AgeCounts, row: Row) => {
          const age = row['en_beach_club_voar'];
          acc[age] = (acc[age] || 0) + 1;
          return acc;
        }, {});
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <h2>Restaurante Beach Club (VOAR)</h2>
