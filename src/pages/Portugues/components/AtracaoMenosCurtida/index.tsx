  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['atracao_nao_curtida'] !== undefined && row['atracao_nao_curtida'] !== '' && parseInt(row['atracao_nao_curtida']) !== 0);
        ...
        const ageCounts: AgeCount = filteredData.reduce((acc: AgeCount, row: Row) => {
          const age = row['atracao_nao_curtida'];
          acc[age] = (acc[age] || 0) + 1;
          return acc;
        }, {});
