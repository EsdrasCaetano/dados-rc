  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['en_buffet_acardia_open_food'] !== undefined && row['en_buffet_acardia_open_food'] !== '' && parseInt(row['en_buffet_acardia_open_food']) !== 0);
        ...
        const ageCounts: AgeCounts = filteredData.reduce((acc: AgeCounts, row: Row) => {
          const age = row['en_buffet_acardia_open_food'];
          acc[age] = (acc[age] || 0) + 1;
          return acc;
        }, {});
