  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['estrutura_evento'] !== undefined && row['estrutura_evento'] !== '' && parseInt(row['estrutura_evento']) !== 0);
        ...
        const ageCounts: AgeCount = filteredData.reduce((acc: AgeCount, row: Row) => {
          const age = row['estrutura_evento'];
          acc[age] = (acc[age] || 0) + 1;
          return acc;
        }, {});
