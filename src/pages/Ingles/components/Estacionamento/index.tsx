  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['en_estacionamento'] !== undefined && row['en_estacionamento'] !== '' && parseInt(row['en_estacionamento']) !== 0);
        ...
        const ageCounts: AgeCounts = filteredData.reduce((acc: AgeCounts, row: Row) => {
          const age = row['en_estacionamento'];
          acc[age] = (acc[age] || 0) + 1;
          return acc;
        }, {});
