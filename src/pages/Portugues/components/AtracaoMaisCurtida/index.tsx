  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['atracoes_mais_curtida'] !== undefined && row['atracoes_mais_curtida'] !== '' && parseInt(row['atracoes_mais_curtida']) !== 0);
        ...
        const ageCounts: AgeCounts = filteredData.reduce((acc: AgeCounts, row: Row) => {
          const age = row['atracoes_mais_curtida'];
          acc[age] = (acc[age] || 0) + 1;
          return acc;
        }, {});
