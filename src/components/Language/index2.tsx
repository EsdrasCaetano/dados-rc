...
useEffect(() => {
  ...
  const processCSV = async () => {
    try {
      console.log('Iniciando processo de busca do CSV...');
      const response = await fetch('../../assets/pesquisarc24.csv');
      const csv = await response.text();
      console.log('CSV convertido em texto:', csv);
      const parsedData: Row[] = Papa.parse(csv, { header: true }).data as Row[];
      console.log('Dados do CSV convertidos:', parsedData);
  
      const column2Data: string[] = parsedData
        .map(row => row['lingua']?.toString()) // Converta os valores para string, pois Papa.parse pode interpretá-los como números
