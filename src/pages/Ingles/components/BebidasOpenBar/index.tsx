...
export function BebidasOpenBar() {
  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['en_bebidas_openbar'] !== undefined && row['en_bebidas_openbar'] !== '' && parseInt(row['en_bebidas_openbar']) !== 0);
        ...
        const ageCounts: AgeCounts = filteredData.reduce((acc: AgeCounts, row: Row) => {
          const age = row['en_bebidas_openbar'];
          acc[age] = (acc[age] || 0) + 1;
          return acc;
        }, {});
