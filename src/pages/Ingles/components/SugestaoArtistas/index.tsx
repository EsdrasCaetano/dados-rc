...
export function SugestaoArtistas() {
  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['en_sugestao_artista'] !== undefined && row['en_sugestao_artista'] !== '' && parseInt(row['en_sugestao_artista']) !== 0);
        ...
        const ageCounts: AgeCounts = filteredData.reduce((acc: AgeCounts, row: Row) => {
          const age = row['en_sugestao_artista'];
          acc[age] = (acc[age] || 0) + 1;
          return acc;
        }, {});
        ...
        const barData = Object.entries(ageCounts)
          ...
          .slice(0, 33); // Seleciona apenas as 15 primeiras entradas
