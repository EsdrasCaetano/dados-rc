  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['en_idade'] !== undefined && row['en_idade'] !== '' && parseInt(row['en_idade']) !== 0);
        ...
        const ageCounts: AgeCounts = filteredData.reduce((acc: AgeCounts, row: Row) => {
          const age = row['en_idade'];
          acc[age] = (acc[age] || 0) + 1;
          return acc;
        }, {});
  
        // Converte os dados agrupados em um array de objetos
        const barData = Object.entries(ageCounts).map(([idade, quantidade]) => ({
          name: idade,
