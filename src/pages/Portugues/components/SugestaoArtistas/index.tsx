  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['sugestao_artista'] !== undefined && row['sugestao_artista'] !== '' && parseInt(row['sugestao_artista']) !== 0);
        ...
        const ageCounts: AgeCounts = filteredData.reduce((acc: AgeCounts, row: Row) => {
          const age = row['sugestao_artista'];
          acc[age] = (acc[age] || 0) + 1;
          return acc;
        }, {});
