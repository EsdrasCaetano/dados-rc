...
export function AtracaoMaisCurtida() {
  ...
  useEffect(() => {
    const processCSV = async () => {
      try {
        ...
        const filteredData = parsedData.filter(row => row['en_atracoes_mais_curtida'] !== undefined && row['en_atracoes_mais_curtida'] !== '' && parseInt(row['en_atracoes_mais_curtida']) !== 0);
        ...
        const ageCounts: AgeCounts = filteredData.reduce((acc: AgeCounts, row: Row) => {
          const age = row['en_atracoes_mais_curtida'];
          acc[age] = (acc[age] || 0) + 1;
          return acc;
        }, {});
